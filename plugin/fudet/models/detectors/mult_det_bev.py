...
class MultDet(MVXTwoStageDetector):
    def __init__(self,
                 use_lidar=True,
                 use_camera=False,
                 use_radar=False,
                 ...
                 init_cfg=None):
        super(MultDet, self).__init__(init_cfg=init_cfg)
    ...
    def extract_img_feat(self, img, img_metas):
        ...
        if self.cam_bev:
            if not isinstance(img_feats_reshaped, torch.Tensor):
                img_feats = img_feats_reshaped[0]
            bev_feats = self.vtransform(img_feats, 
                            img_metas['camera_intrinsics'],
                            img_metas['camera2lidar'], 
                            img_metas['img_aug_matrix'], 
                            img_metas['bev_aug_matrix'])
