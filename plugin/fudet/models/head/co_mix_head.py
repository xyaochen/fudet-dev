...
class CoMixFUTRHead(DETRHead):
    ...
    def __init__(self,
        ...
        self.mix_selection = mix_selection
        self.use_dab = use_dab
        self.anchor_size = anchor_size
        
        super(CoMixFUTRHead, self).__init__(
    ...
    def _init_layers(self):
        ...
        if not self.as_two_stage:
            if not self.use_dab:
                self.query_embed = nn.Embedding(self.num_query, self.embed_dims*2)
            else:
                self.tgt_embed = nn.Embedding(self.num_query, self.embed_dims)
                if not self.mix_selection:
    ...
    def forward(self, mlvl_feats, img_metas):
        ...
        if self.use_aux:
            if len(mlvl_feats) == 2:
                aux_feat = torch.cat(mlvl_feats, dim=1)
                aux_outs = self.aux_head([aux_feat])
            else:
                aux_feat = F.interpolate(mlvl_feats[1], mlvl_feats[0].shape[-2:])
                aux_feat = torch.cat((mlvl_feats[0], aux_feat), dim=1)
                aux_outs = self.aux_head([aux_feat])
        else:
            aux_outs = None
        ...
        query_embeds = None
        if not self.use_dab and not self.mix_selection:
            query_embeds = self.query_embedding.weight
        elif not self.mix_selection:
            tgt_all_embed = tgt_embed = self.tgt_embed.weight           # nq, 256
            refanchor = self.refpoint_embed.weight      # nq, 2
            query_embeds = torch.cat((tgt_embed, refanchor), dim=1)
        else:
            init_anchors = []
            for i, aux_dict in enumerate(aux_outs):
                batch_heatmap = aux_dict[0]['heatmap'].sigmoid()
                batch_reg = aux_dict[0]['reg']
                batch_hei = aux_dict[0]['height']
                init_anchor = self.decode(batch_heatmap, batch_hei, batch_reg, max_num=60*batch_heatmap.shape[1])
                init_anchors.append(init_anchor.detach())
            
            tgt_all_embed = tgt_embed = self.tgt_embed.weight           # nq, 256
            refanchor = torch.cat(init_anchors, dim=1)
            
            refanchor[..., 0:1] = (refanchor[..., 0:1]-self.pc_range[0]) / (self.pc_range[3]-self.pc_range[0])
            refanchor[..., 1:2] = (refanchor[..., 1:2]-self.pc_range[1]) / (self.pc_range[4]-self.pc_range[1])
            refanchor[..., 2:3] = (refanchor[..., 2:3]-self.pc_range[2]) / (self.pc_range[5]-self.pc_range[2])
            refanchor = refanchor.clamp(min=0.0, max=1.0)
            
            refanchor = inverse_sigmoid(refanchor)
            tgt_embed = tgt_embed.unsqueeze(0).expand(batch_size, -1, -1)
            query_embeds = torch.cat((tgt_embed, refanchor), dim=2)
    ...
    def decode(self,
        ...
        scores, inds, clses, ys, xs = self._topk(heat, K=max_num)
        ...
        if reg is not None:
            ...
            xs = xs.view(batch, max_num, 1) + 0.5
            ys = ys.view(batch, max_num, 1) + 0.5
        ...
        xs = xs.view(
            batch, max_num,
            1) * self.train_cfg['out_size_factor'] * self.train_cfg['voxel_size'][0] + self.pc_range[0]
        ys = ys.view(
            batch, max_num,
            1) * self.train_cfg['out_size_factor'] * self.train_cfg['voxel_size'][1] + self.pc_range[1]

        final_scores = scores
        final_preds = clses
        final_anchor = torch.cat([xs, ys, hei], dim=2)
        return final_anchor
