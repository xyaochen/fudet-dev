...
from plugin.fudet.models.utils.dino_components import prepare_for_cdn, dn_post_process, compute_dino_loss

@HEADS.register_module()
class DINOFUTRHead(DETRHead):
    ...
    def __init__(self,
        ...
        super(DINOFUTRHead, self).__init__(
    ...
    def _init_layers(self):
        ...
        # dn label enc
        self.label_enc = nn.Embedding(self.num_classes + 1, self.embed_dims)  # # for indicator
    ...
    def forward(self, mlvl_feats, img_metas, dn_args=None):
        """Forward function.
        Args:
        ...
        input_query_label, input_query_bbox, attn_mask, dn_meta = \
            prepare_for_cdn(dn_args, self.training, self.num_query, self.num_classes,
                           self.embed_dims, self.label_enc)
        tgt_all_embed = tgt_all_embed.unsqueeze(0).repeat(batch_size, 1, 1)
        refanchor = refanchor.unsqueeze(0).repeat(batch_size, 1, 1)
        if input_query_label is not None:
            input_query_label = torch.cat((input_query_label, tgt_all_embed), dim=1)
            input_query_bbox = torch.cat((input_query_bbox, refanchor), dim=1)
            query_embeds = torch.cat((input_query_label, input_query_bbox), dim=2)
        else:
            query_embeds = torch.cat((tgt_all_embed, refanchor), dim=2)
        ...
        # dn post process
        outputs_classes, outputs_coords, dn_meta = dn_post_process(outputs_classes, outputs_coords, dn_meta)

        return outputs_classes, outputs_coords, dn_meta
    ...
    def loss(self,
        ...
        for loss_cls_i, loss_bbox_i in zip(losses_cls[:-1],
            ...
            num_dec_layer += 1
        dn_losses = compute_dino_loss(dn_meta)
