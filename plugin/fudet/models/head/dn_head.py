...
from plugin.fudet.models.utils.dn_components import prepare_for_dn, dn_post_process, compute_dn_loss

@HEADS.register_module()
class DNFUTRHead(DETRHead):
    ...
    def __init__(self,
        ...
        super(DNFUTRHead, self).__init__(
    ...
    def _init_layers(self):
        ...
        if not self.as_two_stage:
            if not self.use_dab:
                ...
                self.tgt_embed = nn.Embedding(self.num_query, self.embed_dims-1)
        ...
        # dn label enc
        self.label_enc = nn.Embedding(self.num_classes + 1, self.embed_dims - 1)  # # for indicator
    ...
    def forward(self, mlvl_feats, img_metas, dn_args=None):
        ...
        input_query_label, input_query_bbox, attn_mask, mask_dict = \
            prepare_for_dn(dn_args, tgt_all_embed, refanchor, batch_size, self.training, self.num_query, self.num_classes,
        ...
        # dn post process
        outputs_classes, outputs_coords = dn_post_process(outputs_classes, outputs_coords, mask_dict)

        return outputs_classes, outputs_coords, mask_dict
