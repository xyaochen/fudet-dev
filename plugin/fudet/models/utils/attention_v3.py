...
class FutrAttentionV3(BaseModule):
    ...
    def __init__(self,
        ...
        if self.fused_embed > embed_dims:
            if self.use_camera and self.use_lidar and self.use_radar:
                self.fused_embed = self.fused_embed - self.embed_dims
                self.all_fusion_layer = nn.Sequential(
    ...
    def forward(self,
        ...
        if self.use_lidar and self.use_camera and self.use_radar:
            ...
            output = torch.cat((pts_output, output), dim=2)
            output = self.all_fusion_layer(output)
...
def feature_sampling_3D(mlvl_feats, reference_points, pc_range):
    ...
    for lvl, feat in enumerate(mlvl_feats):
        ...
        reference_points_rel_lvl = reference_points_rel.view(B, num_query, 1, 2)
        sampled_feat = F.grid_sample(feat, reference_points_rel_lvl)
        sampled_feat = sampled_feat.view(B, C, num_query, num_points)
        sampled_feats.append(sampled_feat)
    sampled_feats = torch.stack(sampled_feats, -1)
    sampled_feats = sampled_feats.view(B, C, num_query, num_points, len(mlvl_feats))
