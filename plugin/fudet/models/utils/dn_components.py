...
def prepare_for_dn(dn_args, tgt_weight, embedweight, batch_size, training, num_queries, num_classes, hidden_dim, label_enc):
    ...
    :param embedweight: positional anchor queries [nq, 4]
    :param batch_size: bs
    ...
    refpoint_emb = embedweight  # [num_query, anchor_size]
    if training:
        ...
        unmask_bbox = unmask_label = torch.cat(known)   # number of gt in the batch
        labels = torch.cat([t['labels'] for t in targets])  # [N_gt]
        boxes = torch.cat([t['boxes'] for t in targets])    # [N_gt, 9]
        batch_idx = torch.cat([torch.full_like(t['labels'].long(), i) for i, t in enumerate(targets)]) # [N_gt] ({0}*{N of first sample}, {1}*{N of second sample} ...)

        known_indice = torch.nonzero(unmask_label + unmask_bbox)    # [N_gt, 1] (0--N_gt)
        ...
        if box_noise_scale > 0:
            ...
            diff[:, :3] = known_bbox_expand[:, 3:6] / 2
        ...
        input_label_embed = label_enc(m)    # [N_gt, embed_dim-1]
        ...
        padding_bbox = torch.zeros(pad_size, refpoint_emb.shape[-1]).cuda()
        # [bs, pad_size+num_query, embed_dims]
...
def tgt_loss_boxes(src_boxes, tgt_boxes, num_tgt, loss_weight=0.25):
    ...
    norm_tgt_boxes = normalize_bbox(tgt_boxes)
    loss_bbox = F.l1_loss(src_boxes, norm_tgt_boxes, reduction='none')

    losses = {}
    losses['tgt_loss_bbox'] = loss_bbox.sum() / num_tgt * loss_weight
    '''
...
def tgt_loss_labels(src_logits_, tgt_labels_, num_tgt, focal_alpha, log=True, loss_weight=2.0):
    ...
    loss_ce = sigmoid_focal_loss(src_logits, target_classes_onehot, num_tgt, alpha=0.25, gamma=2) * src_logits.shape[1]
